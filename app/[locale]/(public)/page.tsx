...
interface ApiCategory { id: number; name: string; slug: string; icon: string | null; color: string }
...
function getCatIconPath(slug: string): string {
  ...
  if (s.includes('aventura'))   return 'M8 3L3 21h18L14 3l-3 5-3-5z'
  ...
  if (s.includes('noctur'))     return 'M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z'
  ...
  return (
    <Link href={`/excursiones/${tour.slug}`}
      ...
      {coverImg ? (
        ...
        <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${catColor}cc, #111111)` }} />
      )}
...
export default async function LandingPage() {
  ...
  return (
    <>
      ...
      {/* ── HERO ─────────────────────────────────────────── */}
      <section className="dt-sec py-16 sm:py-24 px-4 sm:px-6 overflow-hidden">
        <div className="max-w-4xl mx-auto text-center">
          <span className="inline-flex items-center gap-2 bg-accent/8 text-accent text-[11px] font-bold px-4 py-1.5 rounded-full tracking-wider uppercase mb-6">
            <span className="w-1.5 h-1.5 rounded-full bg-accent animate-pulse" />
            Operadora certificada · República Dominicana
          </span>

          <h1 className="font-display font-bold text-[clamp(38px,7vw,76px)] leading-[1.0] tracking-[-0.035em] text-dt-text mb-6" data-dt-key="hero_title">
            {config.hero_title || 'Excursiones auténticas'}<br />
            <span className="text-accent italic">de la isla.</span>
          </h1>

          <p className="text-dt-text-3 text-lg sm:text-xl mb-10 max-w-2xl mx-auto leading-relaxed" data-dt-key="hero_subtitle">
            {config.hero_subtitle}
          </p>

          <div className="flex flex-wrap gap-3 justify-center mb-10">
            <Link href={heroCtaUrl}
              className="inline-flex items-center gap-2 bg-accent text-white font-bold text-base px-8 py-3.5 rounded-lg hover:bg-accent/90 active:scale-95 transition-all shadow-sm">
              <span data-dt-key="hero_cta">{config.hero_cta}</span>
            </Link>
            <a href={`https://wa.me/${config.wa_number}`} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 border border-dt-border text-dt-text-2 font-semibold text-base px-6 py-3.5 rounded-lg hover:bg-dt-bg-2 transition-all">
          ...
          {/* Trust badges */}
          <div className="flex flex-wrap gap-x-6 gap-y-3 justify-center text-sm text-dt-text-3 mb-8">
            {[
              { icon: 'M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z', label: '+820 reseñas 4.9★' },
              { icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z', label: 'Guías locales certificados' },
              { icon: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z', label: 'Cancelación gratuita 48h' },
              { icon: 'M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0z M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h1m8-1a1 1 0 01-1 1H9m4-1V8a1 1 0 011-1h2.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V16a1 1 0 01-1 1h-1m-6-1a1 1 0 001 1h1M5 17a2 2 0 104 0m-4 0a2 2 0 114 0m6 0a2 2 0 104 0m-4 0a2 2 0 114 0', label: 'Recogida puerta a puerta' },
            ].map(({ icon, label }) => (
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {categories.slice(0, 6).map(cat => (
                <Link key={cat.id} href={`/excursiones?cat=${cat.slug}`}
                  className="flex items-center gap-1.5 border border-dt-border text-dt-text-3 text-xs font-semibold px-3 py-1.5 rounded-full hover:border-accent/40 hover:text-dt-text hover:bg-dt-bg-2 transition-all duration-200">
      ...
      {/* ── EDITORIAL GRID ───────────────────────────────── */}
      {editorial.length >= 3 && (
        <section data-reveal className="dt-sec py-16 px-4 sm:px-6">
          <div className="max-w-7xl mx-auto">
            <div className="flex items-end justify-between gap-4 mb-8">
              <div>
                <p className="text-xs font-bold uppercase tracking-widest text-accent mb-1">Lo más reservado</p>
                <h2 className="font-display font-bold text-dt-text text-3xl sm:text-4xl">
                  Experiencias <span className="text-accent">imperdibles</span>
                </h2>
              </div>
              <Link href="/excursiones" className="text-sm font-bold text-dt-text-3 hover:text-accent transition-colors whitespace-nowrap hidden sm:block">
                Ver todas &rarr;
              </Link>
            </div>
      ...
      {/* ── PROMO BANNER 1 ───────────────────────────────── */}
      <section className="dt-sec pt-8 pb-8 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
      ...
      {/* ── CATEGORIES ───────────────────────────────────── */}
      {categories.length > 0 && (
        <section data-reveal className="dt-sec py-16 overflow-hidden">
          <div className="max-w-7xl mx-auto">
            <div className="px-4 sm:px-6 mb-8">
              <p className="text-xs font-bold uppercase tracking-widest text-accent mb-1">Tipos de experiencia</p>
              <h2 className="font-display font-bold text-dt-text text-3xl sm:text-4xl">Explora por actividad</h2>
            </div>
            <div className="flex gap-4 overflow-x-auto no-scrollbar px-4 sm:px-6 pb-2">
              ...
              <Link href="/excursiones"
                className="shrink-0 w-44 h-64 rounded-xl border-2 border-dashed border-dt-border flex flex-col items-center justify-center gap-2 text-dt-text-3 hover:border-accent hover:text-accent transition-colors">
      ...
      {/* ── PROMO BANNER 2 ───────────────────────────────── */}
      <section className="dt-sec px-4 sm:px-6 py-8">
        <div className="max-w-7xl mx-auto">
          <a href={banner2.btn_url ? banner2.btn_url : `https://wa.me/${config.wa_number}?text=Hola%2C%20me%20interesa%20cotizar%20un%20tour%20privado`}
            ...
            <div className="relative">
              ...
              <h3 className="font-display font-black text-white text-2xl sm:text-3xl mb-1.5">{banner2.title}</h3>
              <p className="text-white/60 text-sm">{banner2.sub}</p>
            </div>
            <span className="relative shrink-0 bg-accent text-white font-bold text-sm px-7 py-3 rounded-lg group-hover:bg-white/94 group-hover:text-dt-text transition-colors whitespace-nowrap">
              {banner2.btn_text} →
            </span>
          </a>
        </div>
      ...
      {/* ── MORE TOURS ───────────────────────────────────── */}
      {moreTours.length > 0 && (
        <section className="dt-sec py-16 overflow-hidden">
          <div className="max-w-7xl mx-auto">
            <div className="px-4 sm:px-6 mb-8 flex items-end justify-between">
              <div>
                ...
                <h2 className="font-display font-bold text-dt-text text-3xl sm:text-4xl">Más por descubrir</h2>
              </div>
              <Link href="/excursiones" className="text-sm font-bold text-dt-text-3 hover:text-accent transition-colors whitespace-nowrap">
                Ver catálogo &rarr;
              </Link>
            </div>
            <div className="flex gap-4 overflow-x-auto no-scrollbar px-4 sm:px-6 pb-2">
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex items-end justify-between gap-4 mb-8">
              <div>
                ...
                <h2 className="font-display font-bold text-dt-text text-3xl sm:text-4xl">Destinos en la isla</h2>
              </div>
              <Link href="/excursiones" className="text-sm font-bold text-dt-text-3 hover:text-accent transition-colors whitespace-nowrap hidden sm:block">
                Ver todos &rarr;
              </Link>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {zones.map((zone, i) => (
                <Link key={zone} href={`/excursiones?zone=${encodeURIComponent(zone)}`}
                  ...
                  className="group flex items-center justify-between p-4 border border-dt-border rounded-xl hover:border-accent/30 hover:shadow-dt transition-all duration-200">
                  <div className="min-w-0">
                    <p className="font-display font-bold text-dt-text text-sm leading-tight truncate">{zone}</p>
                    {zoneCounts[zone] && (
                      <p className="text-dt-text-3 text-xs mt-0.5">{zoneCounts[zone]} {zoneCounts[zone] === 1 ? 'tour' : 'tours'}</p>
                    )}
                  </div>
                  <svg className="w-4 h-4 text-dt-border group-hover:text-accent/60 transition-colors shrink-0 ml-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7"/>
                  </svg>
                </Link>
              ))}
      ...
      {/* ── WHY US ───────────────────────────────────────── */}
      <section className="dt-sec py-16 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          <div className="mb-10">
            <p className="text-accent text-xs font-bold uppercase tracking-widest mb-2">¿Por qué elegirnos?</p>
            <h2 className="font-display font-bold text-dt-text text-3xl sm:text-4xl">La diferencia<br />Dominicana Tour</h2>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="lg:col-span-1 bg-dt-bg-2 border border-dt-border rounded-xl p-8 flex flex-col justify-between">
              ...
              <div>
                <h3 className="font-display font-bold text-dt-text text-2xl mb-3">Guías locales certificados</h3>
                <p className="text-dt-text-3 text-sm leading-relaxed">
                  Dominicanos nativos con certificación del Ministerio de Turismo. Conocen cada rincón de la isla y hablan tu idioma.
                </p>
              </div>
              <div className="mt-6 pt-6 border-t border-dt-border flex items-center gap-2">
            ...
            <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-4">
              {[
                { icon: 'M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z', title: 'Reserva 100% segura', desc: 'Confirmación inmediata. Solo 20% de anticipo. Cancelación gratuita hasta 48h antes.' },
                { icon: 'M9 17a2 2 0 11-4 0 2 2 0 014 0zM19 17a2 2 0 11-4 0 2 2 0 014 0z M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10a1 1 0 001 1h1m8-1a1 1 0 01-1 1H9m4-1V8a1 1 0 011-1h2.586a1 1 0 01.707.293l3.414 3.414a1 1 0 01.293.707V16a1 1 0 01-1 1h-1m-6-1a1 1 0 001 1h1M5 17a2 2 0 104 0m-4 0a2 2 0 114 0m6 0a2 2 0 104 0m-4 0a2 2 0 114 0', title: 'Puerta a puerta', desc: 'Recogida y retorno desde tu hotel. Sin estrés, sin costos ocultos.' },
                { icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z', title: 'Grupos pequeños', desc: 'Máx. 15-20 personas. Más atención, más autenticidad.' },
              ].map(({ icon, title, desc }) => (
                <div key={title} className="bg-dt-bg-2 hover:bg-dt-bg border border-dt-border rounded-xl p-6 transition-all duration-200 group">
                  ...
                  <h4 className="font-display font-bold text-dt-text text-base mb-2">{title}</h4>
                  <p className="text-dt-text-3 text-sm leading-relaxed">{desc}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ── TESTIMONIALS (dark photo bg, no dt-sec) ──────── */}
      <section className="relative py-20 overflow-hidden mb-1.5">
      ...
      {/* ── CTA ──────────────────────────────────────────── */}
      <section className="dt-sec bg-accent py-16 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="text-white/75 text-lg mb-8 max-w-md mx-auto">{config.why_cta_text}</p>
