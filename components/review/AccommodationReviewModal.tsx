...
function ReviewForm({ accommodationSlug, accommodationName }: Props) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setSub(true)
    try {
      const r = await fetch(`${API}/v3/accommodations/${accommodationSlug}/reviews`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ firstName: firstName.trim(), country, rating, comment: comment.trim(), language: 'es' }),
      })
    ...
    >
      <div className="w-full sm:max-w-md bg-dt-surface border border-dt-border rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-dt-border">
          <div>
            <p className="font-display font-bold text-dt-text text-base leading-tight">¿Cómo estuvo tu estadía?</p>
            <p className="text-dt-text-3 text-xs mt-0.5 truncate max-w-[240px]">{accommodationName}</p>
          </div>
          <button onClick={close} className="w-8 h-8 rounded-full flex items-center justify-center text-dt-text-3 hover:bg-dt-border transition-colors" aria-label="Cerrar">
        ...
        {done ? (
          <div className="px-6 py-10 text-center">
            ...
            <p className="text-dt-text-3 text-sm leading-relaxed">Tu opinión está pendiente de moderación y aparecerá pronto.</p>
            <button onClick={close} className="mt-6 px-6 py-2.5 bg-accent text-white text-sm font-semibold rounded-xl hover:bg-accent/90 transition-colors">
              Cerrar
            </button>
          </div>
        ) : (
          <form onSubmit={submit} className="px-5 py-5 flex flex-col gap-4">
            <div className="flex flex-col items-center gap-2 py-2">
              ...
              <div className="flex gap-1" onMouseLeave={() => setHover(0)}>
                {[1,2,3,4,5].map(s => (
                  <button key={s} type="button" onMouseEnter={() => setHover(s)} onClick={() => setRating(s)} className="p-0.5 transition-transform hover:scale-110 active:scale-95" aria-label={`${s} estrella${s > 1 ? 's' : ''}`}>
                    <svg className="w-9 h-9 transition-colors" viewBox="0 0 24 24"
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="text" value={firstName} onChange={e => setName(e.target.value)} placeholder="María" maxLength={60}
              ...
              <div>
                ...
                <select value={country} onChange={e => setCountry(e.target.value)}
                  ...
                  <option value="DO">RD</option><option value="US">EE.UU.</option><option value="CA">Canadá</option>
                  <option value="MX">México</option><option value="CO">Colombia</option><option value="VE">Venezuela</option>
                  <option value="PR">Puerto Rico</option><option value="GB">Reino Unido</option>
                  <option value="DE">Alemania</option><option value="FR">Francia</option><option value="RU">Rusia</option>
                  <option value="BR">Brasil</option><option value="AR">Argentina</option><option value="OTHER">Otro</option>
                </select>
              </div>
            </div>
            ...
            <div>
              ...
              <textarea value={comment} onChange={e => setComment(e.target.value)}
                placeholder="Cuéntanos cómo fue tu estadía, qué te gustó más..." rows={3} maxLength={1000}
...
export function AccommodationReviewModal(props: Props) {
  return (
